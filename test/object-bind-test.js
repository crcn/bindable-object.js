...
describe("object-bind#", function () {
  ...
  it("can bind now to a defined value", function (next) {
    var obj = new BindableObject({ name: "steph" });
    obj.bind("name", function (value) {
  ...
  it("can call 'now' multiple times", function () {
    var obj = new BindableObject({ name: "steph" }), called = 0;
  ...
  it("can dispose a binding", function () {
    var obj = new BindableObject(), calls = 0;
  ...
  it("can bind to a property owned by another bindable object", function () {
    var friend1 = new BindableObject({ name: "joe" });
    var friend = new BindableObject({ name: "josh", friend:  friend1 }),
  ...
  it("can dispose a sub-bound value", function () {
    var friend = new BindableObject({ name: "craig" }),
    obj = new BindableObject({ friend: friend }),
    boundValue;
