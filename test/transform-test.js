...
describe("transform#", function () {
  ...
  it("throws an error if 'to' is an unknown type", function () {
    var err;
    try {
      new BindableObject().bind("abba", { to: 0 }).now();
    } catch(e) {
      err = e;
    }
  ...
  it("can bind a property both ways", function () {
    var obj = new BindableObject({ name: "frank" });
    obj.bind("name", {
      to: "name2",
      bothWays: true
    }).now();
  ...
  it("can transform to another bindable object", function () {
    var obj = new BindableObject({ name: "sam" }),
  ...
  it("can map multiple properties", function (next) {
    var obj = new BindableObject({ first: "a", last: "b" });
    obj.bind("first, last", {
      map: function (a, b) {
        return a + " " + b;
      },
  ...
  it("can listen to a binding once", function () {
    var obj = new BindableObject({ name: "a" }), calls = 0;
  ...
  it("can listen to a expect.jsn once", function () {
    var obj = new BindableObject({ a: { b: 1 }}), calls = 0;
  ...
  it("can set a max number of calls", function () {
    var obj = new BindableObject({ a: 1 }), calls = 0;
  ...
  it("doesn't set undefined property to another property that is defined", function () {
    var obj = new BindableObject({ b: 5 });
    obj.bind("a", { to: "b" }).now();
    obj.bind("b", { to: "c" }).now();
