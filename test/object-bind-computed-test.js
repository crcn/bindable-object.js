...
describe("object-computed#", function () {
  ...
  it("can explicitly define what properties to watch on a function", function (next) {
    ...
    //obj.context(obj);
    obj.friends = friends;
    obj.eachFriend = BindableObject.computed(["friends"], function (fn) {
      friends.forEach(fn);
    });
  ...
  it("does not return the context of a value that is a bindable object if watched", function () {
    var friend;
    var obj = new BindableObject({ a: {friends: [{ name: "sam" }, { name: "monica"}, { name: "tim" } ].map(function(v) { return new BindableObject(v); }) } });
